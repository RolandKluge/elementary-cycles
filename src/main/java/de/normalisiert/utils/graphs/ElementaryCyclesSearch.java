 ...
 * graph nodes implementation. Each of these lists represents an elementary
 * cycle.<br>
 * <br>
 *
 ...
 * SIAM Journal on Computing. Volume 4, No. 1 (1975), pages 77-84.<br>
 * <br>
 *
...
public class ElementaryCyclesSearch<T> {
	...
	private List<Vector<Integer>> B = null;

	/** Stack for nodes, used by the algorithm of Johnson */
	private Vector<Integer> stack = null;

	/**
	 * Initializes this algorithm with an adjacency matrix and the corresponding
	 * node objects
	 *
	 * @param matrix
	 *            adjacency-matrix of the graph
	 * @param graphNodes
	 *            array of the graph nodes of the graph; this is used to build sets
	...
	public List<Vector<T>> getElementaryCycles() {
		...
		this.B = new ArrayList<Vector<Integer>>(this.adjList.length);
	...
	private boolean findCycles(int v, int s, Vector<Integer>[] adjList) {
		...
		if (f) {
			...
			for (int i = 0; i < adjList[v].size(); i++) {
				final int w = ((Integer) adjList[v].get(i)).intValue();
				final Vector<Integer> bList = this.B.get(w);
	...
	private void unblock(int node) {
		...
		Vector<Integer> Bnode = this.B.get(node);
		while (Bnode.size() > 0) {
