...
/**
 * This is a help class for the search of all elementary cycles in a graph with
 ...
 * adjacency-list of a graph. Based on this graph, it gets a node number s, for
 * which it calculates the subgraph, containing all nodes {s, s + 1, ..., n},
 * where n is the highest node number in the original graph (e.g. it builds a
 * subgraph with all nodes with higher or same node numbers like the given node
 * s). It returns the strong connected component of this subgraph which contains
 * the lowest node number of all nodes in the subgraph.<br>
 * <br>
 *
 ...
 * @author Roland Kluge - Introduce generics
...
public class StrongConnectedComponents {
	...
	/**
	 * This method returns the adjacency-structure of the strong connected component
	 * with the least vertex in a subgraph of the original graph induced by the
	 * nodes {s, s + 1, ..., n}, where s is a given node. Note that trivial strong
	 * connected components with just one node will not be returned.
	 *
	 * @param node
	 *            node s
	 * @return SCCResult with adjacency-structure of the strong connected component;
	...
	private void makeAdjListSubgraph(int node) {
		...
		for (int i = node; i < this.adjList.length; i++) {
			final Vector<Integer> successors = new Vector<>();
	...
	/**
	 * @return Vector[]::Integer representing the adjacency-structure of the strong
	 *         connected component with least vertex in the currently viewed
	 *         subgraph
	 */
	private Vector<Integer>[] getAdjList(Vector<Integer> nodes) {
		Vector<Integer>[] lowestIdAdjacencyList = null;

		if (nodes != null) {
			lowestIdAdjacencyList = createIntVectorArray(this.adjList.length);
			for (int i = 0; i < lowestIdAdjacencyList.length; i++) {
				lowestIdAdjacencyList[i] = new Vector<Integer>();
			}
	...
	private Vector<Integer>[] createIntVectorArray(int length) {
		return (Vector<Integer>[]) new Vector<?>[length];
	}
